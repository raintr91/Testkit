...
export function stateDir(): string {
  if (process.env.TESTKIT_STATE_DIR) return path.resolve(process.env.TESTKIT_STATE_DIR)
...
function rawRepos(): string[] {
  ...
  try {
    ...
    return [...new Set(repos.filter((repo): repo is string => typeof repo === 'string').map(normalize))]
  } catch {
    return []
  }
}
